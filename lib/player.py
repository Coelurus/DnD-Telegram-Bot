...
class Player:
    ...
    def get_actions(self, current_characters: ModifiedPeople) -> tuple[list[dict[str, dict[str, str|int]]], list[ModifiedNPC]]:
        """
    ...
    def get_relationships(self, people_here: list[ModifiedNPC]) -> dict[int, int]:
    ...
    def unequip_weapon(self, item: Item) -> None:
        """Method to put players equiped weapon back to the inventory"""
        self.equiped_weapons.remove(item.ID)
        self.items.append(item.ID)

    
    def get_equiped_weapons(self) -> list[Item]:
        """Returns list of currently equiped weapons as Item objects"""
        return [ItemsCollection.get_item(item_ID) for item_ID in self.equiped_weapons]
        ...
        for quest_idx in range(len(self.quests)):
            ...
            if self.progress[quest_idx] == "infinal":
                ...
                if quest.action != "plant" or quest.action != "rob":
                    if quest.item_ID in self.items:
                        self.remove_item(quest.item_ID)
